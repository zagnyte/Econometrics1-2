...
Šiame kode naudojamos funkcijos iš paketo **"prob"**.

```{r}
#surandame visas 3 kauliukų ridenimo eksperimento galimas baigtis ir jų teorines tikimybes
St <- rolldie(3, nsides = 6, makespace = TRUE)
#prijungiame stulpelį su iškritusių akučių sumomis
St <- addrv(St, sum = X1 + X2 + X3)
#randame visų galimų iškritusių akučių sumų teorines tikimybes
...
```{r}
cbind(as.data.frame(table(S)/n), S_teor$probs)
```
...
#suskaičiuojame kaip keitėsi empirinė dispersija didėjant n
#skaičiuojame tik kas dešimtame taške, nes tai neturi didelės įtakos tikslumui, bet skaičiavimai užtrunka trumpiau nei skaičiuojant dispersiją kiekviename taške
a = rep(0, n/10)
for(i in 1:10000){
  a[i]=var(S[1:(i*10)])
}
